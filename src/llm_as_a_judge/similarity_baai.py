...
OUTPUT_DIR = "../../llm_as_a_judge/results/similarity_baai"
...
BATCH_SIZE = 8  # Batch-Größe für GPU-Verarbeitung
...
def create_visualizations(df, output_dir, experiment_name):
    ...
    sns.boxplot(data=df_exp, x='llm_model', y='mean_similarity')
    plt.title(f'Verteilung der Cosine Similarity - {experiment_name}')
    plt.xlabel('Modell')
    ...
    if len(df['experiment'].unique()) > 1:
        ...
        plt.title('Cosine Similarity: Prompt- und Modellvergleich')
        plt.xlabel('Modell')
        plt.ylabel('Mittlere Cosine Similarity')
        plt.xticks(rotation=45, ha='right')
        plt.legend(title='Promptmethode')
        plt.tight_layout()
    ...
    sns.heatmap(pivot_table, annot=True, fmt='.4f', cmap='YlOrRd', cbar_kws={'label': 'Mittlere Cosine Similarity'})
    plt.title('Mittlere Cosine Similarity nach Modell und Experiment')
    plt.tight_layout()
...
def create_comparison_boxplots(df, output_dir):
    ...
    for emb_model in df['embedding_model'].unique():
        ...
        plt.title(f'Cosine Similarity: Prompt- und Modellvergleich\nEmbedding-Modell: {emb_model}')
    ...
    # Gesamtvergleich: Alle Embedding-Modelle zusammen (hier nur eines)
    if len(df['embedding_model'].unique()) > 1:
        ...
        plt.title('Cosine Similarity: Vergleich über alle Embedding-Modelle')
        plt.xlabel('LLM-Modell (Embedding-Modell)')
        plt.ylabel('Mittlere Cosine Similarity')
        plt.xticks(rotation=45, ha='right')
        plt.legend(title='Promptmethode')
        plt.tight_layout()
...
def create_summary_statistics(df, output_dir):
    ...
    with open(output_path / 'summary_statistics.txt', 'w') as f:
        f.write("=== COSINE SIMILARITY ANALYSIS (BAAI BGE M3) ===\n\n")
...
def main():
    print("Starte Similarity-Analyse mit BAAI/bge-m3...")
