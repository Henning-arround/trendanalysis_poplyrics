...
embeddings_path = os.path.join(base_path, "embedding")
...
# =============================================================================
# PARAMETER - Diese Werte anpassen um verschiedene Modelle zu testen
# =============================================================================
N_NEIGHBORS = 70
N_COMPONENTS = 5
...
print(f"Parameters: n_neighbors={N_NEIGHBORS}, n_components={N_COMPONENTS}, "
      ...
      f"epsilon={CLUSTER_SELECTION_EPSILON}, outlier_threshold={OUTLIER_THRESHOLD}")

# Configure UMAP (identisch zu dtm_evaluation.py)
umap_model = UMAP(
...
# Configure HDBSCAN (identisch zu dtm_evaluation.py)
...
# Re-initialize embedding model on CPU for BERTopic fitting (matches dtm_evaluation.py)
...
# Create BERTopic model (identisch zu dtm_evaluation.py)
...
# Display basic statistics before outlier reduction
...
# Reduce outliers (identisch zu dtm_evaluation.py)
print("\nReducing outliers...")
try:
    # Use probabilities strategy for soft-clustering (identisch zu dtm_evaluation.py)
    ...
    topic_model.update_topics(documents, topics=new_topics, vectorizer_model=vectorizer_model, top_n_words=30)
    
    # Explicitly update the model's topics attribute so the reduced topics are saved
...
# Display basic statistics after outlier reduction
print(f"\nAfter outlier reduction:")
print(f"Number of topics found: {len(set(topics)) - (1 if -1 in topics else 0)}")
print(f"Number of outliers: {sum(1 for t in topics if t == -1)}")

# Save the model
bertopic_model_path = os.path.join(model_path, "bertopic_model_dtm")
topic_model.save(bertopic_model_path)
print(f"\nModel saved to {bertopic_model_path}")
...
print(f"Number of outliers: {n_outliers} ({n_outliers/len(documents):.2%})")
